...
export default function ManagerAddCustomer({ navigation, route }) {
    ...
    useEffect(() => { 
        const unsubscribe = navigation.addListener('focus',() => {//מאזין כל פעם שהוא נכנס לדף ומפעיל את הפונקציות הנבחרות
            ...
            setID(user.KitchenManagersCode)
            setName(user.Name)
            setEmail(user.Email)
    ...
    const UpdateCustomer = async () => {
        let user = {
                ...
                Email: email,
                Password: password,
                KitchenManagersCode:id
            }
        await fetch(`${ServerApi()}/api/UpdateKitchenManagers`, {
            method: 'POST',
            ...
            .then((result) => {
                console.log("fetch POST", JSON.stringify(result))
                    Alert.alert("כל הכבוד", "עדכון מנהל מטבח בוצע בהצלחה")
    ...
    return (
        <View style={{ backgroundColor: "#FFF", height: '100%' }}>
            ...
            <View style={styles.header}>
                ...
                <Image
                    source={require('../assets/logoS2/m.png')}
                    resizeMode='contain'
                    style={{
                        marginTop: 50,
                        width: '55%',
                ...
                <TouchableOpacity onPress={() => { userVal() }}>
                    <View style={[styles.button, { marginTop: 30, }]} >
                        <LinearGradient
                            ...
                            <Text style={styles.textlogin}>          ערוך מנהל מטבח          </Text>
                        </LinearGradient >
                    </View>
                </TouchableOpacity>
